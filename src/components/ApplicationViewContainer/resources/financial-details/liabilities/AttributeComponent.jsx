...
import {
	APP_FIELDS_DICT,
	CURRENCY_FIELDS,
} from '../../../dictionary/application-fields-dictionary';
export const AttributeComponent = ({ propName, propValue }) => {
	return (
		<Box display={'flex'}>
			<Box>
				<Typography fontWeight={'bold'}>
					{APP_FIELDS_DICT[propName]}:{' '}
				</Typography>
			</Box>
			<Box ml={2}>{renderValue(propValue)}</Box>
		</Box>
	);
};
			...
			<Typography fontSize={'12px'}>
				{propValue &&
					STRING_HELPERS.capitalizeFirstLetter(
						APP_FIELDS_DICT[propValue.toString()] || propValue.toString()
					)}
			</Typography>
		);
	}
	...
	return (
		<Box display="flex">
			<Typography fontSize={12} fontWeight={'bold'}>
				{APP_FIELDS_DICT[keyLabel]}:
			</Typography>
			<Typography fontSize={12} ml={1}>
				{formatFragment(keyLabel, APP_FIELDS_DICT[value] || value)}
			</Typography>
		</Box>
	);
};
