    ...
    for (const [key, value] of Object.entries(data)) {
      ...
      if (key === SIGNUP_FIELDS.dateOfBirth) {
        if (value && value.toISOString) {
          ...
          console.warn(
            'Data store error (PUT) - date format is not a dayjs object '
          );
        }
      } else if (
        key === SIGNUP_FIELDS.unitNumber ||
        key === SIGNUP_FIELDS.additionalAddress
      ) {
      ...
      if (key === SIGNUP_FIELDS.dateOfBirth) {
        ...
        if (storedDateValue) {
          ...
          if (!daysObject.toISOString) {
            ...
            console.error(
              'Data store (GET) - cannot convert from string to dayjs object'
            );
            obj[key] = null;
          } else {
            obj[key] = daysObject;
