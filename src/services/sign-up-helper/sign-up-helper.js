...
export class SignUpHelper {
  ...
  static async run(fields) {
    ...
    const registrationResult = await SignUpHelper.#attemptToRegister({
      firstName,
      lastName,
      email,
      password,
    ...
    if (
      registrationResult === SIGNUP_FLOW_RESULT_STATUS.success ||
      registrationResult === SIGNUP_FLOW_RESULT_STATUS.emailExists
    ) {
  ...
  static async #LoginAndGetLinkToken({ email, password }) {
    ...
    if (jwtTokenData && jwtTokenData.tok) {
      ...
      const linkToken = await SignUpHelper.#getLinkToken({
        authToken: jwtTokenData.tok,
      });
   ...
   */
  static async #attemptToRegister({
    email,
    firstName,
    lastName,
    password,
    ...
    try {
      const authClient = new AuthClient();
      await authClient.createUserAccount({
        firstName,
        lastName,
        email,
        password,
      ...
      if (error.response?.status === 400) {
        if (
          error.response?.data?.code?.startsWith(
            SIGNUP_FLOW_RESULT_STATUS.emailExists
          )
        ) {
          return SIGNUP_FLOW_RESULT_STATUS.emailExists;
        } else if (
          error.response?.data?.code?.startsWith(
            SIGNUP_FLOW_RESULT_STATUS.invalidUsernamePassword
          )
        ) {
