...
function ApplicationsPage (props) {
    ...
    if (jwtToken) {
      try {
        ...
        if (data.applications) {
          const userData = await Promise.allSettled(data.applications.map((app) => adminClient.adminGetUserById(app.requestedBy)));
          const groupedData = groupApplicationsAndUsers(userData, data.applications)
  ...
  return (
    <>
      ...
      <Typography mt={3} variant="h3" textAlign={"center"}>Applications</Typography>
      <StyledApplicationsPageContainer>
        <ApplicationsList userApplications={userApplications} onApplicationClicked={handleApplicationClicked} />
      </StyledApplicationsPageContainer>
    </>
  )
}
...
const groupApplicationsAndUsers = (userData, applications) => {
  return applications.map((application) => {
    const specificUser = userData.find((u) => u.value && u.value.id === application.requestedBy)
    return {
      application,
