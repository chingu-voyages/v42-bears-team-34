      ...
      if (jwtToken) {
        if (user?.id && pendingApplicationId) {
          try {
            ...
            const applicationResponseData = await userClient.getApplicationById(pendingApplicationId);
            const userResponseData = await userClient.getUserById(TokenManager.parseToken(jwtToken).id);
    ...
    <Box>
      <StyledSummaryBoxContainer>
        <Box component={"header"} mt={3}>
          <Typography variant="h3" textAlign={"center"}>Thank you for completing the application!</Typography>
          <Box mt={3}>
            <Typography variant="h5">Your application will be reviewed by a member of our team,
            and you will receive a response via e-mail within 24-48 business hours.
            You can also log in with the credentials you entered in this application to the <Link style={{ textDecoration: "none", fontSize: "20px" }} to="/login">USER PORTAL </Link> 
            to check the status of your application.
            Below is a summary of your application.</Typography>
          </Box>
        </Box>
        <Box component={"div"}>
